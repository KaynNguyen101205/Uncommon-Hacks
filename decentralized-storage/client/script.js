...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // ==============================
  // Global Variables & Secret Info
  // ==============================
  // We'll store the latest AES key and CID in localStorage
  window.lastAESKey = localStorage.getItem("aesKey") || "";
  window.lastCID = localStorage.getItem("fileCID") || "";
  ...
  function updateSecretInfo() {
    ...
    if (window.lastAESKey && aesKeyDisplay) {
      aesKeyDisplay.innerText = window.lastAESKey;
    }
    if (window.lastCID && fileCIDDisplay) {
  ...
  if (loginButton) {
    loginButton.addEventListener('click', async () => {
      ...
      if (account) {
        ...
        // Optionally redirect to logedIn.html if desired:
  ...
  if (encryptButton) {
    encryptButton.addEventListener('click', async () => {
      ...
      reader.onload = async function(event) {
        ...
        window.lastAESKey = exportedKeyHex;
        localStorage.setItem("aesKey", window.lastAESKey);
        const result = {
          iv: bufferToHex(iv),
        ...
        const downloadEncryptedButton = document.getElementById('downloadEncryptedButton');
        const uploadPinataButton = document.getElementById('uploadPinataButton');
        if (downloadEncryptedButton) downloadEncryptedButton.disabled = false;
        if (uploadPinataButton) uploadPinataButton.disabled = false;
      };
  ...
  async function uploadToPinata(jsonContent) {
    ...
    try {
      ...
      window.lastCID = result.IpfsHash;
      localStorage.setItem("fileCID", window.lastCID);
      updateSecretInfo();
      return result;
    } catch (error) {
  ...
  const uploadPinataButton = document.getElementById('uploadPinataButton');
  if (uploadPinataButton) {
    uploadPinataButton.addEventListener('click', async () => {
      if (!encryptedFileJSON) {
        alert("No encrypted file available. Please encrypt a file first.");
        return;
      }
  ...
  const downloadDecryptedButton = document.getElementById('downloadDecryptedButton');
  if (downloadDecryptedButton) {
    downloadDecryptedButton.addEventListener('click', async () => {
  ...
  const registerFileButton = document.getElementById("registerFileButton");
  if (registerFileButton) {
    registerFileButton.addEventListener("click", async () => {
  ...
  // Secret Info Toggle
  // ==============================
  const toggleSecretButton = document.getElementById("toggleSecretButton");
    ...
    toggleSecretButton.addEventListener("click", () => {
      ...
      if (secretDiv) {
        if (secretDiv.style.display === "none") {
          ...
          toggleSecretButton.innerText = "Hide Secret Info";
        } else {
          secretDiv.style.display = "none";
          toggleSecretButton.innerText = "Show Secret Info";
        }
      }
    });
  }
